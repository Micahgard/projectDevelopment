                        ...
                        $("#getInvoice").change(function() {
                            ...
                            while (i < data.length) {
                                if (data[i].AdmissionID == $("#getInvoice").val()) {
                                    ...
                                    $("#invoice").append("<table class='table grid'><thead><tr><th>Doctor(s)</th><th>Fee(s)</th></thead></table>");

                                    d = 0;
                                    fee = 0;
                                    while (d < data[i].doctor.length) {
                                        $("#invoice").append("<table class='table grid'><tbody><td>" + data[i].doctor[d].firstname + " " + data[i].doctor[d].lastname + "</td><td>" + data[i].doctor[d].fee + "</td></tr></tbody></table>");
                                        fee += parseFloat(data[i].doctor[d].fee);
                                        d++;
                                    }

                                    due = subtotal + fee;
                                    $("#invoice").append("<table class='table grid'><tr><td>Total Due:</td><td>$" + due + "</td></tr></table>");
                                }
                                i++;
                            }
                        });
